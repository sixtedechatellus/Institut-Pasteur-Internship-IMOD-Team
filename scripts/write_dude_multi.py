...
def read_smi_mol(path):
    ...
    for l in mols_lines:
        parts = l.strip().split()
        if len(parts) >= 2:
            smis.append(parts[1])   # decoy = 2ème colonne
        elif len(parts) == 1:
            smis.append(parts[0])   # actif = 1 seule colonne
...
def parser(protein_path, mol_path, ligand_path, activity, pocket_index, raid=6):
    ...
    #Essai, liges en dessous celles de base
    ...
    #for i in range (len(protein_coord)):
    #    for j in range (len(ligand_coord)):
    #        if np.linalg.norm(protein_coord[i] - ligand_coord[j]) < 6:
    #            pocket_atom_idx.append(i)
    #            break
...
if __name__ == '__main__':
    print("[DEBUG] mol_data_path =", args.mol_data_path)
    print("[DEBUG] contenu:", os.listdir(args.mol_data_path))

    # détection automatique : racine avec plusieurs sous-dossiers ou sous-dossier unique
    subdirs = [
        x for x in os.listdir(args.mol_data_path)
        if os.path.isdir(os.path.join(args.mol_data_path, x))
        and x != "ligands"]

    if len(subdirs) == 0:
        # on est déjà dans un dossier cible (pas de sous-dossiers à parcourir)
        protein_path = [os.path.join(args.mol_data_path, 'receptor.pdb')]
        act_mol_path = [os.path.join(args.mol_data_path, 'actives_final.ism')]
        decoy_mol_path = [os.path.join(args.mol_data_path, 'decoys_final.ism')]
    else:
        # dossier racine : on parcourt les sous-dossiers
        protein_path = [os.path.join(args.mol_data_path, x, 'receptor.pdb') for x in subdirs]
        act_mol_path = [os.path.join(args.mol_data_path, x, 'actives_final.ism') for x in subdirs]
        decoy_mol_path = [os.path.join(args.mol_data_path, x, 'decoys_final.ism') for x in subdirs]    
    ...
    for i, pocket in tqdm.tqdm(enumerate(protein_path)):
        ...
        # write pocket
        d = pocket_parser(pocket, pocket.replace('receptor.pdb', 'crystal_ligand.mol2'), i, raid = 6.0)
